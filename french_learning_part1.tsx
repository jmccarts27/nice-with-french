...
const FrenchLearning = () => {
  ...
  const handleNext = () => {
    setFlipped(false);
    setCurrentCardIndex((currentCardIndex + 1) % filteredData.length);
  };

  const handlePrev = () => {
    setFlipped(false);
    setCurrentCardIndex((currentCardIndex - 1 + filteredData.length) % filteredData.length);
  };
  ...
  const generateQuizOptions = () => {
    ...
    const correctAnswer = filteredData[Math.floor(Math.random() * filteredData.length)];
  ...
  const handleQuizAnswer = (option) => {
    ...
    if (option.french === quizOptions[0].french) {
      const newLearned = new Set(learned);
      newLearned.add(option.french);
      setLearned(newLearned);
    } else {
      const newReview = new Set(needsReview);
      newReview.add(quizOptions[0].french);
      setNeedsReview(newReview);
    }
  ...
  const speak = (text) => {
    ...
    const frenchVoice = voices.find(voice => voice.lang.includes('fr') && voice.name.includes('Male')) || voices.find(voice => voice.lang.includes('fr'));
  ...
  useEffect(() => {
    const handleKeyPress = (e) => {
      if (mode === 'flashcard') {
        ...
        else if (e.key === 'l') markLearned();
        else if (e.key === 'r') markNeedsReview();
      } else if (mode === 'quiz' && quizOptions.length > 0) {
        if (e.key === '1') handleQuizAnswer(quizOptions[0]);
  ...
  }, [mode, flipped, currentCardIndex, quizOptions, selectedAnswer]);
  ...
  const Dashboard = () => (
    <div className="space-y-6">
      <div className="grid md:grid-cols-2 gap-4">
        <div className="bg-gradient-to-br from-blue-500 to-indigo-600 text-white p-6 rounded-lg shadow-lg cursor-pointer hover:scale-105 transition" onClick={() => setMode('browse')}>
        ...
        <div className="bg-gradient-to-br from-purple-500 to-pink-600 text-white p-6 rounded-lg shadow-lg cursor-pointer hover:scale-105 transition" onClick={() => setMode('flashcard')}>
          <Zap className="w-12 h-12 mb-4" />
          <h2 className="text-2xl font-bold mb-2">Flashcard Mode</h2>
          <p>Practice with flip cards and track your progress</p>
        </div>
        
        <div className="bg-gradient-to-br from-green-500 to-teal-600 text-white p-6 rounded-lg shadow-lg cursor-pointer hover:scale-105 transition" onClick={() => setMode('quiz')}>
        ...
        <div className="bg-gradient-to-br from-orange-500 to-red-600 text-white p-6 rounded-lg shadow-lg cursor-pointer hover:scale-105 transition" onClick={() => setMode('stats')}>
      ...
      <div className="bg-white p-6 rounded-lg shadow">
        ...
        <div className="grid md:grid-cols-4 gap-3">
          <button onClick={() => setSessionMode('quick')} className={`p-3 rounded ${sessionMode === 'quick' ? 'bg-indigo-600 text-white' : 'bg-gray-100'}`}>
            ⚡ Quick (Top 20)
          </button>
          <button onClick={() => setSessionMode('full')} className={`p-3 rounded ${sessionMode === 'full' ? 'bg-indigo-600 text-white' : 'bg-gray-100'}`}>
            📚 Full Session
          </button>
          <button onClick={() => setSessionMode('review')} className={`p-3 rounded ${sessionMode === 'review' ? 'bg-indigo-600 text-white' : 'bg-gray-100'}`}>
            🔄 Review ({needsReview.size})
          </button>
          <button onClick={() => { setSessionMode('category'); setMode('browse'); }} className={`p-3 rounded ${sessionMode === 'category' ? 'bg-indigo-600 text-white' : 'bg-gray-100'}`}>
            📂 By Category
          </button>
        </div>
      </div>
      ...
      <div className="bg-amber-50 border-l-4 border-amber-500 p-4 rounded">
        ...
        <p className="text-amber-800">Want audio lessons? Check out <a href="https://coffeebreaklanguages.com/tag/coffee-break-french/" target="_blank" rel="noopener noreferrer" className="underline font-semibold">Coffee Break French</a> for excellent podcast lessons!</p>
      </div>
    </div>
  );
  ...
  const BrowseMode = () => (
    <div className="space-y-4">
      <div className="flex gap-3">
        <button onClick={() => setMode('dashboard')} className="px-4 py-2 bg-gray-200 rounded hover:bg-gray-300">
          ← Dashboard
        </button>
        ...
        <select value={categoryFilter} onChange={(e) => setCategoryFilter(e.target.value)} className="px-4 py-2 border rounded">
          {categories.map(cat => (
          ...
          <div key={idx} className="bg-white p-4 rounded-lg shadow hover:shadow-md transition">
            <div className="flex items-start justify-between">
              <div className="flex-1">
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      <button onClick={() => speak(item.french)} className="text-blue-600 hover:text-blue-800">
                ...
                {item.tip && <div className="text-sm text-blue-700 bg-blue-50 p-2 rounded mb-1">💡 {item.tip}</div>}
                {item.cultural && <div className="text-sm text-purple-700 bg-purple-50 p-2 rounded mb-1">🎭 {item.cultural}</div>}
                {item.mistake && <div className="text-sm text-red-700 bg-red-50 p-2 rounded">⚠️ {item.mistake}</div>}
              </div>
              
              <div className="flex flex-col gap-2 ml-4">
                <span className="px-2 py-1 bg-indigo-100 text-indigo-800 text-xs rounded">{item.category}</span>
                <span className="px-2 py-1 bg-yellow-100 text-yellow-800 text-xs rounded">⭐ {item.importance}</span>
                {learned.has(item.french) && <span className="px-2 py-1 bg-green-100 text-green-800 text-xs rounded">✓ Learned</span>}
                {needsReview.has(item.french) && <span className="px-2 py-1 bg-orange-100 text-orange-800 text-xs rounded">↻ Review</span>}
              </div>
            </div>
          </div>
        ))}
  ...
  const FlashcardMode = () => {
    ...
    const progress = `${currentCardIndex + 1} / ${filteredData.length}`;
    ...
    return (
      <div className="space-y-4">
        <div className="flex justify-between items-center">
          ...
          <button onClick={() => { setLearned(new Set()); setNeedsReview(new Set()); }} className="px-4 py-2 bg-red-100 text-red-700 rounded hover:bg-red-200">
        ...
        <div className="relative h-96 perspective-1000">
          <div className={`absolute inset-0 transition-transform duration-500 transform-style-3d ${flipped ? 'rotate-y-180' : ''}`}>
            {/* Front */}
            <div className="absolute inset-0 bg-gradient-to-br from-indigo-500 to-purple-600 rounded-2xl shadow-2xl p-8 flex flex-col items-center justify-center backface-hidden cursor-pointer" onClick={handleFlip}>
              <span className="text-6xl mb-6">{currentCard.emoji}</span>
              <h2 className="text-4xl font-bold text-white mb-4">{currentCard.french}</h2>
              <button onClick={(e) => { e.stopPropagation(); speak(currentCard.french); }} className="text-white hover:text-blue-200">
            ...
            {/* Back */}
            <div className="absolute inset-0 bg-gradient-to-br from-teal-500 to-green-600 rounded-2xl shadow-2xl p-8 flex flex-col items-center justify-center backface-hidden rotate-y-180 cursor-pointer" onClick={handleFlip}>
              <h2 className="text-3xl font-bold text-white mb-4">{currentCard.english}</h2>
              {currentCard.tip && <div className="text-white/90 bg-white/20 p-3 rounded mb-2">💡 {currentCard.tip}</div>}
              {currentCard.cultural && <div className="text-white/90 bg-white/20 p-3 rounded mb-2">🎭 {currentCard.cultural}</div>}
              {currentCard.mistake && <div className="text-white/90 bg-white/20 p-3 rounded">⚠️ {currentCard.mistake}</div>}
            </div>
          </div>
        </div>

        <div className="grid grid-cols-2 gap-4">
          <button onClick={markNeedsReview} className="px-6 py-3 bg-orange-500 text-white rounded-lg hover:bg-orange-600 flex items-center justify-center gap-2">
            <XCircle className="w-5 h-5" /> Needs Review (R)
          </button>
          <button onClick={markLearned} className="px-6 py-3 bg-green-500 text-white rounded-lg hover:bg-green-600 flex items-center justify-center gap-2">
            <CheckCircle className="w-5 h-5" /> Learned (L)
          </button>
        </div>

        <div className="flex justify-between">
          <button onClick={handlePrev} className="px-6 py-3 bg-gray-200 rounded-lg hover:bg-gray-300 flex items-center gap-2">
            <ChevronLeft className="w-5 h-5" /> Previous (←)
          </button>
          <button onClick={handleNext} className="px-6 py-3 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 flex items-center gap-2">
  ...
  // Quiz Mode Component
  const QuizMode = () => {
    if (quizOptions.length === 0) return <div className="text-center py-20">Loading quiz...</div>;
    
    const correctAnswer = quizOptions[0];
      ...
      <div className="space-y-6">
        ...
        <div className="bg-gradient-to-br from-blue-500 to-purple-600 text-white p-12 rounded-2xl text-center shadow-2xl">
          ...
          <div className="flex items-center justify-center gap-4">
            ...
            <button onClick={() => speak(correctAnswer.french)} className="text-white hover:text-blue-200">
              <Volume2 className="w-8 h-8" />
            </button>
          </div>
          ...
          {quizOptions.map((option, idx) => (
            <button
              ...
              className={`p-6 rounded-xl text-xl font-semibold transition ${
                selectedAnswer
                  ? option.french === correctAnswer.french
    ...
    <div className="space-y-4">
      ...
      <div className="grid md:grid-cols-3 gap-4">
        ...
        <div className="bg-blue-100 p-6 rounded-lg">
          <h3 className="text-3xl font-bold text-blue-800">{Math.round((learned.size / frenchData.length) * 100)}%</h3>
      ...
      <div className="space-y-3">
        {Object.entries(stats).map(([category, data]) => (
          <div key={category} className="bg-white p-4 rounded-lg shadow">
            <h3 className="text-lg font-bold text-indigo-900 mb-2 capitalize">{category}</h3>
            <div className="flex gap-4 text-sm">
              <span>Total: {data.total}</span>
              <span className="text-green-700">Learned: {data.learned}</span>
              <span className="text-orange-700">Review: {data.review}</span>
            </div>
            <div className="w-full bg-gray-200 rounded-full h-3 mt-2">
              <div className="bg-green-500 h-3 rounded-full transition-all" style={{ width: `${(data.learned / data.total) * 100}%` }}></div>
            </div>
          </div>
        ))}
      </div>
